	...
	buttonPress(buttonID)
	{
		...
		switch(buttonType)
		{
			case "overworld":
				...
				overworldData.setNewScene(buttonID);
				break;
			case "shop":
				uiToRender = "shop";
...
shopReturn()
{
	...
	setNewScene(activeShop.getReturnTarget());
	activeShop = null;
}

function areItemsAdded()
{
	return activeShop.areItemsAdded();
}

getDisabledMenu()
{
	return activeShop.getDisabledMenu();
}
...
class Scene 
{
	...
	buildScene()
	{
		data.variables.lastSceneType = "overworld";
...
class DockButton
{
	constructor(id = -1, name = "", target = null, targetType = null, condition = null)
	{
		...
		//this should be cleaned up and changed into just conditions. the code in general here is pretty wtf
